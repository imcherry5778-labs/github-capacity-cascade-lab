...
contract() {
  local scenario="$1" dir="$2" s="$dir/k6-summary.json" samples="$dir/samples.jsonl" logical physical retries failure p95 dropped s429 s503 desired current overflow sessions h5 denied final_active final_sessions final_queue pass=false
...
pair_contract() {
  local axis="$1" control="$2" mitigation="$3" a b passed=false
  a="$RESULT/$control/contract.json"; b="$RESULT/$mitigation/contract.json"
  case "$axis" in
    m1) jq -e '.passed' "$a" >/dev/null && jq -e '.passed' "$b" >/dev/null && awk -v a="$(jq .sidecar.active_overflow_delta "$a")" -v b="$(jq .sidecar.active_overflow_delta "$b")" 'BEGIN{exit !(b<a)}' && passed=true;;
    m2) jq -e '.passed' "$a" >/dev/null && jq -e '.passed' "$b" >/dev/null && awk -v a="$(jq .sidecar.active_overflow_delta "$a")" -v b="$(jq .sidecar.active_overflow_delta "$b")" 'BEGIN{exit !(b<a)}' && passed=true;;
    m3) jq -e '.passed' "$a" >/dev/null && jq -e '.passed' "$b" >/dev/null && awk -v a="$(jq .sidecar.active_overflow_delta "$a")" -v b="$(jq .sidecar.active_overflow_delta "$b")" -v c="$(jq .hpa.desired_replicas_max "$a")" -v d="$(jq .hpa.desired_replicas_max "$b")" 'BEGIN{exit !(b<a && d>c)}' && passed=true;;
    m4) jq -e '.passed' "$a" >/dev/null && jq -e '.passed' "$b" >/dev/null && awk -v a="$(jq .sidecar.active_overflow_delta "$a")" -v b="$(jq .sidecar.active_overflow_delta "$b")" 'BEGIN{exit !(b<a)}' && passed=true;;
  esac
  jq -n --arg axis "$axis" --argjson passed "$passed" --argjson control "$(cat "$a")" --argjson mitigation "$(cat "$b")" '{axis:$axis,passed:$passed,control:$control,mitigation:$mitigation,comparison:{one_variable:true,recovery_boundary:"final selected sidecar active and HAProxy queue/sessions equal zero"}}' >"$RESULT/$axis-contract.json"
