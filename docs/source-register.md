...
| COMP-K8S-03 | [Kubernetes Horizontal Pod Autoscaling](https://kubernetes.io/docs/concepts/workloads/autoscaling/horizontal-pod-autoscale/) | HPA control loop, resource/custom metric categories, scaling behavior와 status interpretation | GitHub August 17의 HPA presence, metric, threshold, timing, policy 또는 topology | L05 component semantics only | VERIFIED 2026-09-16; online current docs, selected K3s behavior remains L05 evidence authority |
| COMP-K8S-04 | [HorizontalPodAutoscaler v2 API](https://kubernetes.io/docs/reference/kubernetes-api/autoscaling/horizontal-pod-autoscaler-v2/) | `autoscaling/v2` `ContainerResource`, Pods metric, target type와 behavior field schema | GitHub production resource request, metric, target, min/max replicas 또는 stabilization setting | L05 manifest schema only | VERIFIED 2026-09-16; online current API docs, actual selected server status remains L05 evidence authority |
| COMP-K8S-05 | [Kubernetes API Aggregation Layer](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/) | APIService aggregation boundary와 extension API role | GitHub custom-metrics provider, adapter, RBAC, aggregation TLS policy 또는 production security configuration | L05 minimal adapter boundary only | VERIFIED 2026-09-16; temporary `insecureSkipTLSVerify` is local implementation, not source guidance |
...
| COMP-IST-02 | [Istio Helm install](https://istio.io/latest/docs/setup/install/helm/) | L04/L05 pinned `istio-base` → `istiod` Helm install contract와 local artifact source review | GitHub의 August 17, 2026 exact Istio version, sidecar configuration, concurrency value, chart/image source 또는 production topology를 지원하지 않는다. | L04, L05 | VERIFIED 2026-09-16; selected version 1.30.4; current artifact source is runtime metadata only |
...
| COMP-IST-07 | [EnvoyFilter API](https://istio.io/latest/docs/reference/config/networking/envoy-filter/) / [Istio 1.30.4 analyzer](https://github.com/istio/istio/blob/1.30.4/pkg/config/analysis/analyzers/envoyfilter/envoyfilter.go) / [Istio 1.30.4 route patch](https://github.com/istio/istio/blob/1.30.4/pilot/pkg/networking/core/envoyfilter/rc_patch.go) | 일반 EnvoyFilter API와 upgrade-fragility warning; selected Istio 1.30.4 tagged implementation은 `REPLACE`를 `VIRTUAL_HOST`에 valid하게 처리하고 `priority=0` relative-operation warning을 검사하며, route patch implementation은 matched virtual host를 replacement한다. `HTTP_ROUTE` 경로는 REMOVE/MERGE/INSERT 계열만 처리하므로 L04가 exact `SIDECAR_INBOUND`/`inbound|8080||`/`inbound|http|8080` VIRTUAL_HOST replacement를 선택한 version-specific 근거다. | GitHub August 17의 exact Istio version, sidecar configuration, concurrency mechanism/value, retry policy, EnvoyFilter 사용 여부, exact route/virtual-host configuration 또는 production topology를 지원하지 않는다. | L04 | VERIFIED 2026-08-31; selected Istio 1.30.4 tagged analyzer/route source confirmed; fallback remains version-specific |
| COMP-FOR-01 | [Forgejo v15 release schedule](https://forgejo.org/docs/v15.0/admin/release-schedule/) | v15.0 is LTS and its published schedule; L12 selected 15.0.9 is a reproducibility choice against that documented LTS line | GitHub product/component choice, production HA design or support guarantee for this lab | L12 | VERIFIED 2026-09-19; page identifies v15.0 as LTS and v15.0.9 as LTS docs target |
| COMP-FOR-02 | [Forgejo Actions reference](https://forgejo.org/docs/v15.0/user/actions/reference/) | `workflow_dispatch` API event, remote `uses` URL resolution, explicit fully-qualified URLs and exact commit SHA action version syntax | GitHub Actions parity, GitHub source/action topology or a general security guarantee | L12 | VERIFIED 2026-09-19; selected 15.0.9/13.1.0 runtime behavior is additionally recorded in L12 raw evidence |
| COMP-FOR-03 | [Forgejo Actions runner security](https://forgejo.org/docs/v15.0/admin/actions/runner-security/) | Actions runner security boundary, including the need to treat execution environment as trusted for its executor model | That a disposable container host executor is production multi-tenant isolation or that it has no risk | L12 | VERIFIED 2026-09-19; L12 narrows the host to a disposable container and has no Docker socket/privileged mount |
| COMP-FOR-04 | [Forgejo Generic Package Registry](https://forgejo.org/docs/v15.0/user/packages/generic/) | Generic package PUT/GET endpoint and package file storage semantics used for prepared vendor input and produced artifact | Production artifact retention, signing, attestation, registry HA or supply-chain security | L12 | VERIFIED 2026-09-19; actual 15.0.9 PUT/GET behavior is captured by L12 workflow/package evidence |
| COMP-FOR-05 | [Forgejo Repository Mirrors](https://forgejo.org/docs/v15.0/user/repo-mirror/) | Pull/push mirroring behavior and its operational boundary | Automatic mirroring/failback was used or should be used by L12 | L12 non-goal | VERIFIED 2026-09-19; L12 intentionally uses prepared read-only snapshots only |
| COMP-FOR-06 | [Forgejo Runner installation with Docker](https://forgejo.org/docs/v15.0/admin/actions/installation/docker/) | Official OCI runner image contains `forgejo-runner`, defaults to non-root, and requires configuration/registration before use | The documentation example's Docker-in-Docker/privileged setup is suitable for L12 or is a runner-isolation recommendation | L12 | VERIFIED 2026-09-19; L12 instead uses the exact `13.1.0` image digest, non-root disposable host executor, no Docker socket and no privileged mode |
| COMP-GO-01 | [Go Modules Reference — vendoring](https://go.dev/ref/mod#go-mod-vendor) | `go mod vendor -o`, `vendor/modules.txt`, `-mod=vendor`, and the distinction that `go mod download` remains network/module-cache-oriented | The L12 vendor archive is a complete production dependency/SBOM/attestation mechanism | L12 | VERIFIED 2026-09-19; actual Go 1.26.7 offline test/build is L12 evidence |
| COMP-DKR-01 | [Docker networking — internal networks](https://docs.docker.com/engine/network/#internal-networks) | Internal-network backend communication boundary | That every L12 service lacks egress; L12 measures runner egress only | L12 | VERIFIED 2026-09-19; inspect/probe evidence is authoritative for the selected Docker runtime |
