...
const scenario = __ENV.L09_SCENARIO || __ENV.L06_SCENARIO || 'cascade-no-retry';
if (!supportedScenarios.has(scenario)) {
  throw new Error(`unsupported L09_SCENARIO: ${scenario}`);
}
...
const logicalIDNamespace = __ENV.LOGICAL_ID_NAMESPACE || 'l09-cascade-pair';
...
export const handleSummary = createSummaryHandler({
  ...
  requestPath: __ENV.REQUEST_PATH || 'non-injected k6 Job -> HAProxy -> ClusterIP Service -> istio-proxy -> auth-sim (AKS)',
